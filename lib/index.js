...
const removeAllChecks = () => {
  $('.select-for-meal').prop('checked', false)
}
...
const addFoodToMeal = (meal, food) => {
  ...
  const totalCalorieNode = $(`.meal[data-id=${meal.id}] .total-calories`)
  const totalCalories = parseInt(totalCalorieNode.text() || 0) + parseInt(food.calories)
  totalCalorieNode.text(totalCalories)

  const remainingCaloriesNode = $(`.meal[data-id=${meal.id}] .remaining-calories`)
  const remainingCalories = caloriesGoalForMeal(meal) - totalCalories
...
const caloriesGoalForMeal = (meal) => {
  if(meal.id === 1) {
    return 400
  }
  if(meal.id === 2) {
...
const generateFoodRow = (food) => {
  return `<tr><td class="name">${food.name}</td><td class="calories">${food.calories}</td><td><i class="far fa-trash-alt delete-food"></i></td></tr>`
}
